...
class ExponentialWeighting(WmMethod):
    ...
    def gen_watermarks(self):
        ...
        if self.test_quot:
            ...
            for i in range(0, self.K):
                ...
                if self.save_wm:
                    save_triggerset(trigger_set, os.path.join(self.path, self.dataset, self.arch, 'exponential_weighting', f'C{self.num_classes}_K{self.K}', self.coding), f'{self.runname}_{i+1}')
                    path = os.path.join(os.path.join(self.path, self.dataset, self.arch, 'exponential_weighting', f'C{self.num_classes}_K{self.K}', self.coding), f'{self.runname}_{i+1}')
    ...
    def embed(self, net, criterion, optimizer, scheduler, train_set, test_set, train_loader, test_loader, valid_loader, device, save_dir):
        ...
        self.trigger_set = get_trg_set(os.path.join(self.path, self.dataset, self.arch, 'exponential_weighting', f'C{self.num_classes}_K{self.K}', self.coding, self.runname), 'labels.txt', self.size,
                                                    transform)

        # load trigger indices
        path = os.path.join(self.path, self.dataset, self.arch, 'exponential_weighting', f'C{self.num_classes}_K{self.K}', self.coding, self.runname)
        os.makedirs(path, exist_ok=True)
