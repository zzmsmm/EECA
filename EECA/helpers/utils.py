...
def save_triggerset(trigger_set, path, runname, wm_type=None):
    ...
    for idx, (img, lbl) in enumerate(trigger_set):
        # print(img)
        # torch.save(img, os.path.join(images, str(idx+1) + '.pt'))
...
def get_trg_set_split(path, labels, size, transform=None):
    wm_set = ImageFolderCustomClass(
        path, transform)
    img_nlbl = list()
    wm_targets = np.loadtxt(os.path.join(path, labels))
    ...
    sub_train = range(0, int(size))
    sub_test = range(500, 700)

    train_set = torch.utils.data.Subset(wm_set, sub_train)
    test_set = torch.utils.data.Subset(wm_set, sub_test)
...
def set_up_optim_sched(args, net):
    ...
    if args.sched == 'MultiStepLR_cifar100' or args.sched == 'MultiStepLR_cifar10':
        scheduler = MultiStepLR(optimizer, milestones=[20, 40, 60, 80, 100, 120, 140, 160, 180], gamma=args.lradj)
    elif args.sched == 'MultiStepLR_caltech101':
        scheduler = MultiStepLR(optimizer, milestones=[20, 40, 60, 80, 100, 120, 140, 160, 180], gamma=args.lradj)
    elif args.sched == 'MultiStepLR_tiny-imagenet':
        scheduler = MultiStepLR(optimizer, milestones=[20, 40, 60, 80, 100, 120, 140, 160, 180], gamma=args.lradj)
    elif args.sched == 'MultiStepLR_fashionmnist':
        scheduler = MultiStepLR(optimizer, milestones=[10, 20, 40, 80, 100, 120, 140, 160, 180], gamma=args.lradj) # fashionmnist
    elif args.sched == 'MultiStepLR_mnist':
        scheduler = MultiStepLR(optimizer, milestones=[10, 20, 45, 80, 100, 120, 140, 160, 180], gamma=args.lradj) # mnist
    elif args.sched == 'MultiStepLR':
        scheduler = MultiStepLR(optimizer, milestones=[80, 100, 120, 140, 160, 180], gamma=args.lradj)
