...
parser.add_argument('--coding', default='Full-Entropy', help='coding methods')
...
# experiments
parser.add_argument('--trg_set_sizes_list', nargs='+', default=[1000], type=int, help='trgset sizes for experiments')
...
try:
    ...
    if args.method == 'ProtectingIP':
        log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), 'watermark', str(args.wm_type), f'C{args.num_classes}_K{args.K}', args.coding)
    elif args.method == 'Ood':
        log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), 'watermark', 'ood', f'C{args.num_classes}_K{args.K}', args.coding)
    elif args.method == 'ExponentialWeighting':
        log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), 'watermark', 'exponential_weighting', f'C{args.num_classes}_K{args.K}', args.coding)
    ...
    if args.method == 'ProtectingIP':
        save_dir = os.path.join(cwd, 'checkpoint', str(args.dataset), str(args.arch), 'watermark', str(args.wm_type), f'C{args.num_classes}_K{args.K}', args.coding)
    elif args.method == 'Ood':
        save_dir = os.path.join(cwd, 'checkpoint', str(args.dataset), str(args.arch), 'watermark', 'ood', f'C{args.num_classes}_K{args.K}', args.coding)
    elif args.method == 'ExponentialWeighting':
        save_dir = os.path.join(cwd, 'checkpoint', str(args.dataset), str(args.arch), 'watermark', 'exponential_weighting', f'C{args.num_classes}_K{args.K}', args.coding)
