...
def train(epoch, net, criterion, optimizer, scheduler, train_loader, device, avg_train_losses, avg_valid_losses,
...
def test(net, criterion, loader, device, net_0=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(loader):
            ...
            if net_0:
                net_0.eval()
                _, targets = torch.max(net_0(inputs).data, 1)
            loss = criterion(outputs, targets)
            test_loss += loss.item()
            _, predicted = torch.max(outputs.data, 1)
            total += targets.size(0)
...
def train_wo_wms(epochs, net, criterion, optimizer, scheduler, patience, train_loader, test_loader, valid_loader,
    ...
    for epoch in range(epochs):
        ...
        avg_train_losses, avg_valid_losses, train_acc, valid_acc = train(epoch, net, criterion, optimizer, scheduler, train_loader,
                                                                         device,
...
def train_on_augmented(epochs, device, net, optimizer, criterion, scheduler, patience, train_loader, test_loader,
    ...
    for epoch in range(epochs):
        avg_train_losses, avg_valid_losses, train_acc, valid_acc = train(epoch, net, criterion, optimizer, scheduler,
                                                                         train_loader, device,
                                                                         avg_train_losses, avg_valid_losses,
                                                                         wmloader=wm_loader)
